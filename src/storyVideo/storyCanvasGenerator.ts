...
async function obtainRawCover(
  ...
  // 3b. Try fetching from public website URL
  if (typeof song.coverArt === "string" && !song.coverArt.startsWith("http")) {
    try {
      const cleanPath = song.coverArt.replace(/^\//, "");
      const fullUrl = `https://deybalal.ir/${cleanPath}`;
